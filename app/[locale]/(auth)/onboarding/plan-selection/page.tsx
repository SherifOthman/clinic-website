...
export default function PlanSelectionPage() {
  ...
  useEffect(() => {
    ...
    // Check if user has completed clinic setup
    if (!user.verified) {
  ...
  const handlePlanSelection = async () => {
    ...
    try {
      ...
      // Mark that user should see welcome message on dashboard
      // (This will be shown once and then marked as seen)
      if (typeof window !== 'undefined') {
        localStorage.removeItem(`clinicflow_welcome_shown_${user.id}`);
      }
  ...
  return (
    <div className="min-h-screen bg-default-50 dark:bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Stepper */}
        <Stepper steps={steps} currentStep={3} className="mb-12" />
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
          <Button
            variant="bordered"
            className="flex-1"
            onPress={() => router.push(`/${locale}/onboarding/clinic-setup`)}
          >
          ...
          >
            {isSubmitting ? 'Setting up your clinic...' : 'Complete Setup'}
          </Button>
        </div>
